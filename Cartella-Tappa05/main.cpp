...
int main() {
    ...
    sf::Window window(sf::VideoMode({winWidth, winHeight}), "Sistema Solare - Tappa 05", sf::State::Windowed, settings);
    ...
    // Ho allontanato la telecamera in modo da poter vedere l'orbita
    float theta = 0.5f; 
    float phi = 0.0f;   
    float rho = 8.0f; 
    ...
    std::cout << "--- COMANDI TAPPA 05 ---" << std::endl;
    ...
    while (window.isOpen()) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::L)) glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); 
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::F)) glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); 
        ...
        modelSun = glm::rotate(modelSun, time * 0.2f, glm::vec3(0.0f, 1.0f, 0.0f)); //Il sole ruota su se stesso abbastanza piano
        
        glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(modelSun));
        glUniform3f(glGetUniformLocation(program, "objectColor"), 1.0f, 0.8f, 0.0f); //lo imposto giallo al momento
        ...
        //La terra ruota attorno al sole e si allontana di 4 unità sull'asse X, poi ruota su se stessa molto velocemente e viene rimpicciolita
        //Ruota attorno al Sole ad una velocità di 0.6
        modelEarth = glm::rotate(modelEarth, time * 0.6f, glm::vec3(0.0f, 1.0f, 0.0f)); 
        
        //Si allontana dal centro di 4 sull'asse X
        modelEarth = glm::translate(modelEarth, glm::vec3(4.0f, 0.0f, 0.0f)); 
        
        //Ruota su se stessa ad una velocià di 2.0
        modelEarth = glm::rotate(modelEarth, time * 2.0f, glm::vec3(0.0f, 1.0f, 0.0f)); 
        
        //Viene rimpicciolita del 30% rispetto alla dimensione originale
        modelEarth = glm::scale(modelEarth, glm::vec3(0.3f, 0.3f, 0.3f)); 
        
        glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(modelEarth));
        glUniform3f(glGetUniformLocation(program, "objectColor"), 0.2f, 0.4f, 0.9f); // Blu oceano
        glDrawElements(GL_TRIANGLES, i.size(), GL_UNSIGNED_INT, 0);
