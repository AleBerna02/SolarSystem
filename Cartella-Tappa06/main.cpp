...
int main() {
    ...
    sf::Window window(sf::VideoMode({winWidth, winHeight}), "Sistema Solare - Tappa 06", sf::State::Windowed, settings);
    ...
    std::cout << "--- COMANDI TAPPA 06 ---" << std::endl;
    ...
    while (window.isOpen()) {
        ...
        //Separo le due matrici una per la posizione spaziale della terra nel sistema solare
        glm::mat4 earthOrbit = glm::mat4(1.0f);
        earthOrbit = glm::rotate(earthOrbit, time * 0.6f, glm::vec3(0.0f, 1.0f, 0.0f)); 
        earthOrbit = glm::translate(earthOrbit, glm::vec3(4.0f, 0.0f, 0.0f)); 

        //L'altra matrice aggiunge la rotazione sul proprio asse
        glm::mat4 earthDraw = earthOrbit; // Eredita la posizione
        earthDraw = glm::rotate(earthDraw, time * 2.0f, glm::vec3(0.0f, 1.0f, 0.0f)); // Gira su se stessa
        earthDraw = glm::scale(earthDraw, glm::vec3(0.3f, 0.3f, 0.3f)); // Viene rimpicciolita
        
        glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(earthDraw));
        glUniform3f(glGetUniformLocation(program, "objectColor"), 0.2f, 0.4f, 0.9f); 
        glDrawElements(GL_TRIANGLES, i.size(), GL_UNSIGNED_INT, 0);

        //Disegno la luna come figlia della terra
        glm::mat4 moonDraw = earthOrbit; // Eredita la posizione della terra, se riceve una modifica anche la luna lo riceve
        moonDraw = glm::rotate(moonDraw, time * 1.5f, glm::vec3(0.0f, 1.0f, 0.0f)); // Orbita attorno alla terra
        moonDraw = glm::translate(moonDraw, glm::vec3(0.8f, 0.0f, 0.0f)); // Distanza della luna dalla terra
        moonDraw = glm::scale(moonDraw, glm::vec3(0.08f, 0.08f, 0.08f));
        
        glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(moonDraw));
        glUniform3f(glGetUniformLocation(program, "objectColor"), 0.6f, 0.6f, 0.6f); // Grigio roccia
        glDrawElements(GL_TRIANGLES, i.size(), GL_UNSIGNED_INT, 0);
