...
int main() {
    ...
    sf::Window window(sf::VideoMode({winWidth, winHeight}), "Sistema Solare - Tappa 04 ", sf::State::Windowed, settings);
    ...
    std::cout << "--- COMANDI TAPPA 04 ---" << std::endl;
    ...
    while (window.isOpen()) {
        ...
        //Prima la sposto a sinistra
        model1 = glm::translate(model1, glm::vec3(-1.5f, 0.0f, 0.0f));
        //Poi la faccio ruotare col tempo sul suo asse y più lentamente della rossa
        model1 = glm::rotate(model1, time * 0.5f, glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        //Secondo oggetto Sfera Rossa
        glm::mat4 model2 = glm::mat4(1.0f); 
        //Prima la sposto a destra
        model2 = glm::translate(model2, glm::vec3(1.5f, 0.0f, 0.0f));
        //Poi la faccio ruotare su se stessa più velocemente della gialla
        model2 = glm::rotate(model2, time * 1.5f, glm::vec3(0.0f, 1.0f, 0.0f));

        glUniformMatrix4fv(glGetUniformLocation(program, "model"), 1, GL_FALSE, glm::value_ptr(model2));
        glUniform3f(glGetUniformLocation(program, "objectColor"), 0.8f, 0.2f, 0.2f); 
        glDrawElements(GL_TRIANGLES, i.size(), GL_UNSIGNED_INT, 0);
